...
impl<'a> Parser<'a> {
    pub fn start<T>(&mut self, listener: Arc<Mutex<T>>)
    where
        T: ParserListener + Send + 'a,
    {
        self.parser_state = Arc::new(Mutex::new(ParserState { use_utf8: true }));
        let parser_state_cloned = self.parser_state.clone();
        self.parser_fsm = Gn::<String>::new_scoped(move |mut co| {
            loop {
